...
export class NFTWatcher implements ManagedResource {
  ...
  async processTransaction(
    ...
    if (receipt != null && receipt.logs != null) {
      ...
      outputMessages = await this.abortHandle.race(
        Promise.all(
          transfers.map(t => this.complementWithTokenInfo(t.from, t.to, t.tokenIndex, formattedBlock, blockTime)),
        ),
      );
    }
      ...
      if (e instanceof JsonRpcError) {
        ...
        try {
          ...
          if (decodeResults.length == 1) {
            const decodedRes = decodeResults[0];
            errorTokenURI.decodedData = decodedRes;
          }
  ...
  async fetchMetadata(tokenURI: string): Promise<any> {
    ...
    try {
      ...
      // TODO : test the above determination.
      //process.exit(0);
