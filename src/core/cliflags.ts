...
export const CLI_FLAGS = {
  ...
  'print-config': flags.boolean({
    description:
      'Causes blockchainETL to simply print the configuration merged from config file and CLI flags and exit.',
  }),
  'health-check': flags.boolean({
    hidden: true,
    description:
      'Performs health check for a running blockchainETL process by examining the state file created by that process',
  }),

  'config-file': flags.string({
    char: 'c',
    description:
      'BlockchainETL configuration file to use. If not specified blockchainETL will look for a file ' +
      'called blockchainETL.yaml or blockchainETL.json in the current working directory',
  }),
  ...
  'collect-internal-metrics': flags.boolean({
    ...
    description: 'Enables collection of blockchainETL-internal metrics',
  }),
  'mongo-url': flags.string({
    env: 'MONGO_URL',
    description: 'URL to Mongo',
  }),
  ...
  'hec-reject-invalid-certs': flags.boolean({
    ...
    description: 'Disable to allow HEC client to connect to HTTPS without rejecting invalid (self-signed) certificates',
  }),
  ...
  'mongo-address': flags.string({
    env: 'MONGO_ADDRESS',
    description: 'Address of MongoDB',
  }),
  'mongo-port': flags.string({
    env: 'MONGO_PORT',
    description: 'Port of MongoDB',
  }),
  'mongo-reject-invalid-certs': flags.boolean({
    allowNo: true,
    env: 'MONGO_REJECT_INVALID_CERTS',
    description:
      'Disable to allow Mongo client to connect to HTTPS without rejecting invalid (self-signed) certificates',
  }),
  'mongo-events-index': flags.string({
    env: 'MONGO_EVENTS_INDEX',
    description:
      'Mongo index to send events to. You can alternatively use separate HEC tokens to correctly route your data',
  }),
  'mongo-metrics-index': flags.string({
    env: 'MONGO_METRICS_INDEX',
    description:
      'Mongo index to send metrics to. You can alternatively use separate HEC tokens to correctly route your data',
  }),
  'mongo-internal-index': flags.string({
    env: 'MONGO_INTERNAL_INDEX',
    description:
      'Mongo index to send internal metrics to. You can alternatively use separate HEC tokens to correctly route your data',
  }),

  'mongo-events-token': flags.string({
    env: 'MONGO_EVENTS_TOKEN',
    description:
      'Mongo token to use for sending events. You can alternatively configure different indexes to correctly route your data',
  }),
  'mongo-metrics-token': flags.string({
    env: 'MONGO_METRICS_TOKEN',
    description:
      'Mongo token to use for sending metrics. You can alternatively configure different indexes to correctly route your data',
  }),
  'mongo-internal-token': flags.string({
    env: 'MONGO_INTERNAL_TOKEN',
    description:
      'Mongo token to use for sending internal metrics. You can alternatively configure different indexes to correctly route your data',
  }),
