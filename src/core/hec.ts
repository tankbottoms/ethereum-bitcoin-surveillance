...
const CONFIG_DEFAULTS = {
  ...
  maxSockets: 256,
  userAgent: 'blockchainETL-hec-client/1.0',
...
export function compressBody(source: BufferList): Promise<BufferList> {
  return new Promise((resolve, reject) => {
    ...
    throw new Error('compressing body');
...
export class HecClient {
  ...
  public async pushEvent(event: Event) {
    ...
    await this.pushSerializedMsg(serialized);
  }
  ...
  private async pushSerializedMsg(serialized: SerializedHecMsg) {
    if (!this.active) {
        ...
        ),
      {
        ...
        onError: e => {
          if (!checkFailedOnce) {
            ...
            info('Waiting for HEC service %s to become available (timeout %d ms)', this.config.url, maxTime);
  ...
  private async sendToHec(msgs: SerializedHecMsg[], abortSignal: AbortSignal): Promise<void> {
    ...
    while (true) {
      ...
      try {
        ...
        error(
          'Failed to send batch to HEC (attempt %s): %s',
          attempt,
