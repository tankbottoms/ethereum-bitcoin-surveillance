...
/** Root configuration schema for blockchainETL */
export interface BlockchainETLConfigSchema {
  /** Ethereum node configuration */
  eth: EthereumConfigSchema;
  /**
   * In the output configuration you can specify where blockchainETL will send generated
  ...
  /** Mongo */
  mongo: MongoClientConfigSchema;
  /** Checkpoint configuration - how blockchainETL keeps track of state between restarts */
...
export interface BlockchainETLConfig {
  eth: EthereumConfig;
  output: OutputConfigSchema;
  hec: {
    default: HecConfig;
  ...
  mongo: {
    default: MongoConfig;
    events?: OptionalMongoConfig;
    metrics?: OptionalMongoConfig;
    internal?: OptionalMongoConfig;
  };
...
export interface MongoClientConfigSchema {
  /**
   * Base settings that apply to all HEC clients. Overrides for events, metrics and
   * internal metrics will be layered on top of the defaults and allow for using
   * different HEC tokens, URL or destination index.
   */
  default: MongoConfigSchema;
  /** HEC settings (overrides for `default`) for events sent to Splunk */
  events?: OptionalMongoConfigSchema;
  /** HEC settings (overrides for `default`) for metrics sent to Splunk */
  metrics?: OptionalMongoConfigSchema;
  /** HEC settings (overrides for `default`) for internal metrics sent to Splunk */
  internal?: OptionalMongoConfigSchema;
}
/** General Ethereum configuration including client and transport, defining how blockchainETL talks to the ethereum node */
export interface EthereumConfigSchema {
  /** URL of JSON RPC endpoint */
  url: string;
  /**
   * Network name logged as a field with every event and metric.
   * BlockchainETL will attempt to automatically determine if not specified
...
/**
 * The checkpoint is where blockchainETL keeps track of its state, which blocks have already been processed.
...
export interface AbiRepositoryConfigSchema {
  ...
  /**
   * If enabled, blockchainETL will attempt to decode function calls and event logs using a set of
   * common signatures as a fallback if no match against any supplied ABI definition was found.
   */
  decodeAnonymous: boolean;
  /**
   * If enabled, blockchainETL will attempt to reconcile the shape of struct definitions from decoded tuples
...
/**
 * BlockchainETL checks for each address it encounters whether it is a smart contract by attempting to
 * retrieve the contract code. To reduce the performance hit by this operation, blockchainETL can cache
...
/**
 * BlockchainETL-internal metrics allow for visibility into the operation of blockchainETL itself.
 */
...
/** Settings for blockchainETL connecting to the ethereum node via JSON RPC over HTTP */
...
/** Configurable set of `sourcetype` field values emitted by blockchainETL */
...
export interface MongoOutputConfig {
  type: 'mongodb';
  sourcetypes: SourcetypesSchema;
}
...
export interface HecConfigSchema {
   ...
   * @default `blockchainETL-hec-client/<version>`
   * @see [Use variables in metadata](#metadata-variables)
   */
  userAgent?: string;
  ...
  /**
   * If set to > 0, then blockchainETL will wait for the HEC service to become available for the given amount of time
   * by periodically attempting to request the collector/health REST endpoint. This can be useful when starting
   * Splunk and blockchainETL for example in docker-compose, where Splunk takes some time to start.
   */
...
export interface MongoConfigSchema {
  address?: string;
  port?: string;
  ...
  /**
   * Default set of fields to apply to all events and metrics sent with this Mongo client
   * @see [Use variables in metadata](#metadata-variables)
   */
  defaultFields?: { [k: string]: any };
  /** Maximum number of entries in the Mongo message queue before flushing it */
  maxQueueEntries?: number;
  /** Maximum number of milliseconds to wait before flushing the Mongo message queue */
  flushTime?: DurationConfig;
  /** Maximum number of attempts to send a batch to Mongo. By default this there is no limit */
  maxRetries?: number;
  /** Number of milliseconds to wait before considering an HTTP request as failed */
  timeout?: DurationConfig;
  /** Set to `false` to disable HTTP keep-alive for connections to Splunk */
  requestKeepAlive?: boolean;
  /** Maximum number of sockets Mongo will use (per host) */
  maxSockets?: number;
  /** Wait time before retrying to send a (batch of) Mongo messages after an error */
  retryWaitTime?: WaitTimeConfig;
  /**
   * Enable sending multipe metrics in a single message to Mongo.
   */
  multipleMetricFormatEnabled?: boolean;
  /**
   * If set to > 0, then blockchainETL will wait for the Mongo service to become available for the given amount of time
   * by periodically attempting to request the collector/health REST endpoint. This can be useful when starting
   * Splunk and blockchainETL for example in docker-compose, where Splunk takes some time to start.
   */
  waitForAvailability?: DurationConfig;
...
export interface MongoConfig extends Omit<MongoConfigSchema, 'retryWaitTime'> {
  flushTime?: Duration;
  timeout?: Duration;
  retryWaitTime?: WaitTime;
  waitForAvailability?: Duration;
...
export type OptionalMongoConfigSchema = Partial<MongoConfigSchema>;

export type OptionalHecConfig = Partial<HecConfig>;
export type OptionalMongoConfig = Partial<MongoConfig>;
...
export async function loadConfigFile(
  fileName: string,
  type?: 'json' | 'yaml',
): Promise<DeepPartial<BlockchainETLConfigSchema>> {
  ...
  if (detectedType === 'json') {
    ...
    return loadYaml(fileContents, { filename: fileName }) as any;
  }
...
export const DEFAULT_CONFIG_FILE_NAME = 'blockchainETL.yaml';

export function checkConfig(config: BlockchainETLConfig): string[] {
  const problems = [];
  ...
  if (config.output.type === 'hec') {
    if (config.hec.default.url == null) {
      if (config.hec.events?.url == null) {
        problems.push('No URL for HEC events specified. Use --hec-url or configure via blockchainETL.yaml');
      }

      if (config.nodeMetrics.enabled && config.hec.metrics?.url == null) {
        problems.push('No URL for HEC metrics specified. Use --hec-url or configure via blockchainETL.yaml');
      }

      if (config.internalMetrics.enabled && config.hec.internal?.url == null) {
        problems.push('No URL for HEC internal metrics specified. Use --hec-url or configure via blockchainETL.yaml');
      }
    }
      ...
      if (config.hec.metrics?.defaultMetadata?.index == null && config.hec.metrics?.token == null) {
        problems.push(
          'No index nor hec token specified for metrics. Metrics may not be routed to the correct Splunk index. ' +
            'Use flags --hec-metrics-index or --hec-metrics-token or configure via blockchainETL.yaml',
        );
      }
    }
      ...
      if (config.hec.internal?.defaultMetadata?.index == null && config.hec.internal?.token == null) {
        problems.push(
          'No index nor hec token specified for internal metrics. Metrics may not be routed to the correct Splunk index. ' +
            'Use flags --hec-internal-index or --hec-internal-token or configure via blockchainETL.yaml',
        );
      }
    }
  }
...
export async function loadBlockchainETLConfig(flags: CliFlags, dryRun: boolean = false): Promise<BlockchainETLConfig> {
  const defaultsPath = join(__dirname, '../defaults.blockchainETL.yaml');
  ...
  const parseSpecificHecConfig = (
    _defaults: DeepPartial<HecConfigSchema> | undefined,
    indexFlag: keyof CliFlags,
    tokenFlag: keyof CliFlags,
  ): HecConfig | undefined => {
    const res = removeEmtpyValues({
      defaultFields: _defaults?.defaultFields,
      defaultMetadata: flags[indexFlag]
        ? Object.assign({}, _defaults?.defaultMetadata, { index: flags[indexFlag] })
        : _defaults?.defaultMetadata,
      flushTime: parseDuration(_defaults?.flushTime),
      gzip: _defaults?.gzip,
      maxQueueEntries: _defaults?.maxQueueEntries,
      maxQueueSize: _defaults?.maxQueueSize,
      maxRetries: _defaults?.maxRetries,
      maxSockets: _defaults?.maxSockets,
      multipleMetricFormatEnabled: _defaults?.multipleMetricFormatEnabled,
      requestKeepAlive: _defaults?.requestKeepAlive,
      retryWaitTime: waitTimeFromConfig(_defaults?.retryWaitTime as WaitTimeConfig),
      timeout: parseDuration(_defaults?.timeout),
      token: flags[tokenFlag] ?? _defaults?.token,
      url: _defaults?.url,
      userAgent: _defaults?.userAgent,
      validateCertificate: _defaults?.validateCertificate,
      waitForAvailability: parseDuration(_defaults?.waitForAvailability),
    });
    return isEmpty(res) ? undefined : res;
  };

  const parseOutput = (_defaults?: Partial<OutputConfigSchema>): OutputConfigSchema => {
    switch (_defaults?.type) {
      case 'hec':
        const def = _defaults as Partial<HecOutputConfig>;
        return {
          type: 'hec',
      ...
      case 'file':
        return {
          type: 'file',
          path: (_defaults as Partial<FileOutputConfig>)?.path!,
        };
      case 'null':
        return {
          type: 'null',
  ...
  const config: BlockchainETLConfig = {
    eth: {
    ...
    mongo: {
      default: {
        address: required('mongo-address', defaults.mongo?.default?.address),
        port: required('mongo-port', defaults.mongo?.default?.port),
        defaultFields: defaults.mongo?.default?.defaultFields,
        defaultMetadata: defaults.mongo?.default?.defaultMetadata,
        flushTime: parseDuration(defaults.mongo?.default?.flushTime),
        maxQueueEntries: defaults.mongo?.default?.maxQueueEntries,
        maxRetries: defaults.mongo?.default?.maxRetries ?? Infinity,
        maxSockets: defaults.mongo?.default?.maxSockets,
        multipleMetricFormatEnabled: defaults.mongo?.default?.multipleMetricFormatEnabled,
        requestKeepAlive: defaults.mongo?.default?.requestKeepAlive,
        retryWaitTime: waitTimeFromConfig(defaults.mongo?.default?.retryWaitTime as WaitTimeConfig),
        timeout: parseDuration(defaults.mongo?.default?.timeout),
        waitForAvailability: parseDuration(defaults.mongo?.default?.waitForAvailability),
      },
      events: parseSpecificHecConfig(defaults.mongo?.events, 'mongo-events-index', 'mongo-events-token'),
      metrics: parseSpecificHecConfig(defaults.mongo?.metrics, 'mongo-metrics-index', 'mongo-metrics-token'),
      internal: parseSpecificHecConfig(defaults.mongo?.internal, 'mongo-internal-index', 'mongo-internal-token'),
    },
    ...
    internalMetrics: {
      enabled:
        ...
        (() => {
          ...
          warn(
            'Implicitly disabling blockchainETL-internal metrics collection. No dedicated index or HEC token specified.',
          );
          return false;
        })(),
  ...
  if (problems.length > 0) {
    for (const msg of problems) {
      error('Detected problem in blockchainETL config: %s', msg);
    }
    if (!dryRun) {
      throw new ConfigError(
        problems.length > 1 ? 'Detected multiple problems in blockchainETL configuration, see logs' : problems[0],
      );
    }
  }
