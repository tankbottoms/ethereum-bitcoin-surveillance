...
export interface MetadataVariables {
  /** Hostname of the machine blockchainETL is running on */
  HOSTNAME: string;
  ...
  NETWORK_ID?: string;
  /**
   * The network name supplied via blockchainETL config or auto-detected
  ...
  CHAIN_ID?: string;
  /**
   * The chain name supplied via blockchainETL config or auto-detected
   * for known networks from the chain ID and network ID
   */
  CHAIN?: string;
  /** The blockchainETL PID */
  PID: string;
  /** BlockchainETL version */
  VERSION: string;
  /** The node.js version blockchainETL is running on */
  NODE_VERSION: string;
  /**
...
export function substituteVariablesInHecConfig(
  config: BlockchainETLConfig,
  {
    platformAdapter,
    transportOriginHost,
    blockchainETLVersion,
    nodeVersion = process.version,
    pid = process.pid,
    host = hostname(),
  }: {
    platformAdapter: NodePlatformAdapter;
    blockchainETLVersion: string;
    nodeVersion?: string;
    pid?: number;
    host?: string;
