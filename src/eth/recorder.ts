...
export class ReplayEthTransport implements EthereumTransport {
  ...
  public async send(request: JsonRpcRequest): Promise<JsonRpcResponse> {
    ...
    const reqMatches = (r: Record) => JSON.stringify({ method: r.req.method, params: r.req.params }) === requestJson;
    const recordIdx = this.records.findIndex(reqMatches);
    if (recordIdx < 0) {
      throw new Error(
        `Failed to replay request ${request.method}(${request.params.join(', ')}) - no matching recorded request found`,
      );
    }
