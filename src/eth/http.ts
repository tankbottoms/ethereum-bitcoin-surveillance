...
import { HttpTransportConfig } from '../core/config';
...
const { debug, trace, info } = createModuleDebug('eth:http');
...
export class HttpTransport implements EthereumTransport {
  ...
  public async sendInternal(request: JsonRpcRequest | JsonRpcRequest[]): Promise<JsonRpcResponse | JsonRpcResponse[]> {
    ...
    return {
      ...
      httpClient: this.httpAgent.hasOwnProperty('getCurrentStatus')
        ? // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
          httpClientStats((this.httpAgent as HttpAgent | HttpAgent.HttpsAgent).getCurrentStatus())
        : {},
    };
  }
