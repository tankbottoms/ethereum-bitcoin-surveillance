...
import {
  blockNumber,
  clientVersion as importedClientVersion,
  gasPrice,
  hashRate,
  netVersion,
  peerCount,
...
export async function checkRpcMethodSupport(eth: EthereumClient, req: EthRequest<any, any>): Promise<boolean> {
  try {
    ...
    warn(
      'RPC method %s is not supported by ethereum node: %s',
      req.method,
...
export async function fetchDefaultNodeInfo(ethClient: EthereumClient): Promise<DefaultNodeInfo> {
  const [networkId, chainIdResult] = await Promise.all([ethClient.request(netVersion()), ethClient.request(chainId())]);
  return { networkId, chainId: chainIdResult };
}
...
export class GenericNodeAdapter implements NodePlatformAdapter {
  ...
  public async captureNodeInfo(ethClient: EthereumClient): Promise<NodeInfo> {
    const [defaultInfo, version] = await Promise.all([
      ...
      ethClient.request(importedClientVersion()),
    ]);
