...
import { MongoDBClient } from '../core/mongo';
...
export class InternalStatsCollector implements ManagedResource {
  ...
  constructor(public config: { collect: boolean; collectInterval: number; dest: MongoDBClient; basePrefix?: string }) {
  ...
  private next = async () => {
    ...
    if (this.config.collect) {
      ...
      await this.config.dest.pushMetrics({
        time,
