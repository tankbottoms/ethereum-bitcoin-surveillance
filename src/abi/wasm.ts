...
import { memory } from '../wasm/ethabi/pkg/blockchainETL_ethabi_bg';
...
export function parseFunctionSignature(signature: string): AbiItem {
    const result = unwrapJsResult(parse_function_signature(signature));
    if (result instanceof Object) {
        return { type: 'function', ...result };
    } else {
        throw new Error('Expected object in parseFunction');
    }
}

export function parseEventSignature(signature: string): AbiItem {
    const result = unwrapJsResult(parse_event_signature(signature));
    if (result instanceof Object) {
        return { type: 'event', ...result };
    } else {
        throw new Error('Expected object in parseEvent');
    }
}
