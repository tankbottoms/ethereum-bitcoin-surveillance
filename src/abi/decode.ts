...
export function getInputSize(abi: AbiItemDefinition): DataSize {
    try {
        ...
        throw new DecodeError(
            `Failed to determine input size for ${computeSignature(abi)}: ${e instanceof Error? e.message : 'general error'}`
        );
    }
}
    ...
    for (const [abi, { length, exact }] of abisWithSize) {
        if (dataLength === length && exact) {
            try {
                ...
                lastError = e instanceof Error? e : new Error('general error');
                if (TRACE_ENABLED) {
                    trace(
