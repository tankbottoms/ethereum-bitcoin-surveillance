...
import { BlockSegments, BlockRangeTimestamp } from '../types';
...
export const active: Web3 = new Web3(new Web3.providers.HttpProvider(local_node));
...
export const scan_eth_blocks = async (
  ...
  try {
    const counter = approx_ethereum_blocks_per_day;
    ...
    if (time < target_timestamp) {
      ...
      true && logger.info(`incremented:${next_block_start}`);
      ...
      true && logger.info(`estimated timestamp:${estimate}  target timestamp:${target_date}`);
      if (!is_date_same(estimate, target_date)) {
        next_block_start = next_block_start + counter;
      ...
      next_block_start = increment_block - counter;
      true && logger.info(`decremented:${next_block_start}`);
      start_block = await active?.eth.getBlock(next_block_start, get_block_and_transactions);
      false &&
...
export const convert_date_range_to_eth_block_numbers = async (start_date: string, end_date: string) => {
  try {
    ...
    if (start > end || end > now) throw new Error('Bad parameter check start or end date');
