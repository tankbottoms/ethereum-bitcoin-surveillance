...
function createConfigurationSchemaReference(): string {
    ...
    const entryNodeType = findType('BlockchainETLConfigSchema', program, typeChecker);
...
async function main() {
    ...
    const exampleContent = await readFile(join(__dirname, '../test/config/example1.blockchainETL.yaml'), {
        encoding: 'utf-8',
    });
